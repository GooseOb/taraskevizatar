...
Object.assign(String.prototype, {
  toTaraskConvert(arg1, arg2) {
    return toTaraskConvert(this, arg1, arg2);
  },
  toTarask() {
    return toTarask(this);
  },
  toArab() {
    return toArab(this);
  },
  toLatin(arg) {
    return toLatin(this, arg);
  },
  toJ(arg) {
    return toJ(this, arg);
  },
...
Object.assign(Array.prototype, {
  restoreRegister(arg) {
    return restoreRegister(this, arg);
  },
  toHtmlTags(arg1, arg2) {
    return toHtmlTags(this, arg1, arg2);
  },
});
...
export function toTaraskConvert(text, isHtml, { abc = 0, j = 0 }) {
  ...
  switch (abc) {
    case 0:
      ...
      break;
    case 1:
      textSplit = text.toLatin().split(' ');
      break;
    default:
      textSplit = text.toLowerCase().toArab().split(' ');
  }
  text = text.toLowerCase().toTarask();
  if (j) text = text.toJ(j === 2);
  switch (abc) {
    case 1:
      text = text.toLatin(false);
      break;
    case 2:
      text = text.toArab();
  }
...
function restoreRegister(text, orig) {
  for (let i = 0; i < text.length; i++) {
    ...
    if (!oWord[0] || !isUpCase(oWord[0])) continue;
...
function toHtmlTags(text, orig, abc) {
  for (let i = 0; i < text.length; i++) {
    ...
    while (word[fromStart] === oWord[fromStart]) fromStart++;
...
function toTarask(text) {
  ...
  loop: do {
    for (const key in softers) text = text.replace(softers[key], key);
    for (const key in softers)
      if (key !== '$1дзьдз$2' && softers[key].test(text)) continue loop;
    break;
  } while (true);
...
function toLatin(text, upCase = true) {
  for (const key in latinLetters) text = text.replace(latinLetters[key], key);
  if (upCase) {
...
function toArab(text) {
  for (const key in arabLetters) text = text.replace(arabLetters[key], key);
...
function toJ(text, alwaysJ = false) {
  return text.replace(
    ...
    alwaysJ
      ? ($0, $1, $2) => subtoJ($1, $2)
      : ($0, $1, $2) => (Math.random() >= 0.5 ? subtoJ($1, $2) : $0),
  );
}
